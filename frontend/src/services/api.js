...
export const api = {
  ...
  async getDashboard() {
    ...
    return {
      ...
      roomCount: demoData.capacities.length,
  ...
  async createRoom(room) {
    if (!readDemoMode()) return request('/capacities', { method: 'POST', body: JSON.stringify(room) })
    await wait()
    const newRoom = {
      ...room,
      id: Date.now(),
    ...
    demoData.capacities.unshift(newRoom)
    logEvent('Salon eklendi', `${newRoom.classroom} - ${newRoom.capacity} kapasite`)
