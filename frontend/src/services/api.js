...
const initialData = {
  exams: [
    ...
    },
    {
      ...
      classroom: 'B-104',
      studentCount: 58,
      supervisor: 'Doç. Cem Akın',
      status: 'Planlandı',
    },
    {
      id: 4,
      courseCode: 'YAZ402',
      courseName: 'Yazılım Proje Yönetimi',
      date: '2026-06-04',
      time: '14:00',
      classroom: 'C-301',
      studentCount: 26,
      supervisor: 'Arş. Gör. Ece Yılmaz',
      status: 'Planlandı',
    },
  ],
  capacities: [
  ...
  supervisors: [
    { id: 1, name: 'Dr. Selin Kaya', department: 'Bilgisayar Müh.', examCount: 1, availability: 'Uygun' },
    { id: 2, name: 'Doç. Cem Akın', department: 'Matematik', examCount: 1, availability: 'Uygun' },
    { id: 3, name: 'Arş. Gör. Ece Yılmaz', department: 'Yazılım Müh.', examCount: 1, availability: 'Uygun' },
    { id: 4, name: 'Arş. Gör. Ömer Koç', department: 'Bilgisayar Müh.', examCount: 0, availability: 'Uygun' },
  ],
  logs: [
    { id: 1, time: '27.05.2026 09:12', action: 'Sistem açıldı', detail: 'SQL Server bağlantısı hazır', level: 'Başarılı' },
    { id: 2, time: '27.05.2026 10:24', action: 'Sınav listelendi', detail: '4 sınav kaydı getirildi', level: 'Bilgi' },
...
export const api = {
  ...
  async getDashboard() {
    ...
    return {
      ...
      roomUsage: Math.round((usedSeats / totalSeats) * 100),
      pendingCount: demoData.exams.length - assigned,
      recentExams: clone(demoData.exams.slice(0, 3)),
    }
  },
  ...
  async createExam(exam) {
    if (!readDemoMode()) {
      return request('/exams', { method: 'POST', body: JSON.stringify(exam) })
    }
    ...
    logEvent('Sınav eklendi', `${exam.courseCode} - ${exam.classroom}`)
